# Gera WiFiDaLoja-Agent-Setup.exe — agente cloud-only (sem PHP/MySQL/painel local).
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $PSScriptRoot
$Dist = Join-Path $Root "dist-cloud"
$Stage = Join-Path $Dist "payload"
$Zip = Join-Path $Dist "payload.zip"
$Stub = Join-Path $Dist "SetupCloudStub.exe"
...
Write-Host "Compactando pacote cloud..."
...
Write-Host "Compilando instalador cloud..."
$manifest = Join-Path $PSScriptRoot "app.manifest"
$setupCs = Join-Path $PSScriptRoot "SetupCloud.cs"
...
if ($LASTEXITCODE -ne 0) { throw "Falha ao compilar SetupCloud.cs" }
